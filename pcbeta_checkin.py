...
if __name__ == "__main__":
    if "PB_COOKIE" in os.environ:
        ...
        doneUrl = "https://i.pcbeta.com/home.php?mod=task&do=view&id=149"
        lqurl = "https://i.pcbeta.com/home.php?mod=task&do=draw&id="

        newTaskRes = requests.get(url=pcUrl, headers=pcHeaders).text
        doneTaskRes = requests.get(url=doneUrl, headers=pcHeaders).text
        doingRes = requests.get(url="https://i.pcbeta.com/home.php?mod=task&do=doing&view=all", headers=pcHeaders).text

        msg = pcbetaCheckin() + "\n" + pcbetaReply()
        print(msg)
    else:
        print("未设置PB_COOKIE环境变量")
